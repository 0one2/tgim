...
from .networks.txt_enc import Transformer
...
class TGIM_Model(torch.nn.Module):
	...
	def train_G(self, batch):
		...
		# with torch.no_grad():
		# 	source_w_emb, source_s_emb = self.txt_enc(src_cap, src_mask, src_ids)
		source_w_emb, source_s_emb = self.txt_enc(src_cap, src_mask, src_ids)
		...
		fake_c_prob = self.netD.TaGAN_D(fake_features, n_w_embs, n_mask, D_sent_fc=self.cfg.D_sent_fc, negative=False)
	...
	def train_D(self, batch):
		...
		real_c_prob, real_c_prob_n = self.netD.TaGAN_D(src_features, source_w_emb, src_mask, D_sent_fc=self.cfg.D_sent_fc,
		                                               negative=True)
	...
	def eval_G(self, batch):
		...
		with torch.no_grad():
			source_w_emb, source_s_emb = self.txt_enc(src_cap, src_mask, src_ids)
			target_w_emb, target_s_emb = self.txt_enc(tar_cap, tar_mask, tar_ids)
		# Image data preprocess
		src_img_emb = self.netG.encoder(src_img)
		
		
		recon,_ = self.netG.decoder(src_img_emb, source_s_emb)
		mani, _ = self.netG.decoder(src_img_emb, target_s_emb)
